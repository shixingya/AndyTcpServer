...
void TcpServer::incomingConnection(qintptr socketDescriptor) //多线程必须在此函数里捕获新连接
{
   ...
   // connect(this,&TcpServer::sentDisConnect,tcpTemp,&TcpSocket::disConTcp);//断开信号
    ...
    emit createNewTask(tcpTemp);
    connect(tcpTemp,SIGNAL(UpdateText(QString)),this,SIGNAL(UpdateText(QString)));
    connect(tcpTemp,SIGNAL(UpdateMaximum(int)),this,SIGNAL(UpdateMaximum(int)));
    connect(tcpTemp,SIGNAL(UpdatePercent(int)),this,SIGNAL(UpdatePercent(int)));
